    ...
    for i, data in enumerate(data_loader):
        ...
        for idx in index:
            ...
            if gn in tmp:
                ...
                tmp.append(gn)
            n += 1
            dct[gn.split('_')[2]] += 1  # 相同 query id 不同 file 的次数
                ...
                if gname_list_cnt[0][1] >= 5:
                    ...
                    if isinstance(frame_box,np.ndarray):
                        if len(frame_box.shape) == 1:
                            tmp_dct["item_box"] = frame_box.tolist()
                        else:
                            tmp_dct["item_box"] = frame_box[0].tolist()
                    else:
                        tmp_dct["item_box"] = frame_box

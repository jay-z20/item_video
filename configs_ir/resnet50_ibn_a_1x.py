...
# dataset settings
dataset_type = 'CocoIndDataset'
data_root = '/myspace/'
img_norm_cfg = dict(
    mean=[170.81, 160.72, 158.88], std=[58.56, 62.05, 62.11], to_rgb=True)
train_pipeline = [
    dict(type='LoadImageFromFile',with_mask=True),
    ...
    dict(type='Collect', keys=['img', 'gt_bboxes', 'gt_labels'],meta_keys=('filename', 'ori_shape', 'img_shape', 'pad_shape',
                            'scale_factor', 'flip', 'img_norm_cfg','idx')),
]
test_pipeline = [
    ...
    #dict(type='DefaultFormatBundle'),
    dict(type='Collect', keys=['img', 'gt_bboxes', 'gt_labels'],meta_keys=('filename', 'ori_shape', 'img_shape', 'pad_shape',
                            'scale_factor', 'flip', 'img_norm_cfg','ori_box'))
    # dict(
...
log_level = 'INFO'
work_dir = '/myspace/work_dirs/resnet50_ibn_a_ws'

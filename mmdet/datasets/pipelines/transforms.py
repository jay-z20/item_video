        ...
        if self.size is not None:
            if isinstance(results['img'],list):
                ...
                for im in results['img']:
                    tmp.append(mmcv.impad(im, self.size))
                padded_img = tmp
            else:
            ...
            if isinstance(results['img'],list):
                tmp = []
                for im in results['img']:
                    tmp.append(mmcv.impad_to_multiple(im, self.size_divisor, pad_val=self.pad_val))
                padded_img = tmp
            else:
                padded_img = mmcv.impad_to_multiple(
                    results['img'], self.size_divisor, pad_val=self.pad_val)
            if results['mask'] is not None:
                padded_img[results['mask'] == 0] = 0
            # import cv2, os, copy
            # p = '/root/code/myspace/a/'
            # img = copy.copy(padded_img)
            # # if not os.path.exists(p):
            # #     os.mkdir(p)
            # print(len(results['gt_bboxes']),results['gt_bboxes'])
            # for box in results['gt_bboxes']:
            #     x1, y1, x2, y2 = box
            #     x1, y1, x2, y2 = map(int, [x1, y1, x2, y2])
            #     cv2.rectangle(img, (x1, y1), (x2 , y2), (0, 255, 0), 4)
            # cv2.imwrite(p+os.path.basename(results['filename']),img)
            # print(p+os.path.basename(results['filename']))
...
@PIPELINES.register_module
class QueryAug(object):
    def __init__(self,test_mode=False,mean=None):
        self.test_mode = test_mode
        p = 0.5
        self.augmentations = Compose([
            albumentations.RandomBrightnessContrast(p=p),
            #albumentations.CLAHE(p=p),
            albumentations.HueSaturationValue(hue_shift_limit=10, sat_shift_limit=30, val_shift_limit=20, p=p),
            albumentations.GaussianBlur(p=p),
        ], p=p)
        self.randomerasing = RandomErasing(sl=0.1,sh=0.3,mean=mean)

        self.augmentations2 = Compose([
            albumentations.RandomBrightnessContrast(p=p),
            albumentations.HueSaturationValue(hue_shift_limit=10, sat_shift_limit=30, val_shift_limit=20, p=p),
        ], p=p)
        self.randomerasing2 = RandomErasing(mean=mean)

    def __call__(self, results):
        img = results['img']
        if not self.test_mode:
            if 'v_' in results['filename']:
                data = {'image': img}
                aug_img = self.augmentations(**data)
                results['img'] = self.randomerasing(aug_img['image'])
            elif 'i_' in results['filename']:
                data = {'image': img}
                aug_img = self.augmentations2(**data)
                results['img'] = self.randomerasing2(aug_img['image'])
...
@PIPELINES.register_module
class CenterTestCrop(object):
    def __init__(self, crop_size, box_aug=False):
    ...
    def __call__(self, results):
        img = results['img']
        ih, iw = img.shape[:2]
        gt_bboxes = results['gt_bboxes']  ## min_x,min_y,max_x,max_y
        #print('transforms CenterCrop',len(gt_bboxes))
        ...
        ori_box = [x1, y1, x2, y2]
        #w, h = x2 - x1 + 1, y2 - y1 + 1
        # crop 区域 x1 - offw,y1 - offh x2 + offw, y2 + offh
        offset_h = 32
        offset_w = 32
        crop_x1 = max(x1 - offset_w,0)
        crop_y1 = max(y1 - offset_h, 0)
        crop_x2 = min(x2 + offset_w, iw)
        crop_y2 = min(y2 + offset_h, ih)
        crop_x1, crop_y1, crop_x2, crop_y2 = map(int,[crop_x1, crop_y1, crop_x2, crop_y2])
        im = img[crop_y1:crop_y2, crop_x1:crop_x2, :]
        ...
        ih, iw = im.shape[:2]
        crop_h, crop_w = max(self.crop_size), min(self.crop_size)
        rr = min(crop_h / ih, crop_w / iw)
        if crop_h / ih > 2 and crop_w / iw > 2:
            im = mmcv.imrescale(im,2)
            x1, y1, x2, y2 = map(lambda x: x * 2, [x1, y1, x2, y2])
        elif rr > 1.0:
            im = mmcv.imrescale(im, rr)
            x1, y1, x2, y2 = map(lambda x: x * rr, [x1, y1, x2, y2])
        elif ih > crop_h or iw > crop_w:
            r = min(crop_h / ih, crop_w / iw)
            h_n, w_n = min(int(r * ih), crop_h), min(int(r * iw), crop_w)
            im = mmcv.imresize(im, (w_n, h_n))
            x1, y1, x2, y2 = map(lambda x: x * r, [x1, y1, x2, y2])
        tmp_box.append([x1, y1, x2, y2])

        img_shape = im.shape
        results['img'] = im
        results['img_shape'] = img_shape
        results['gt_bboxes'] = np.array(tmp_box, dtype=np.float32)
        results['ori_box'] = ori_box
        return results
...
class CenterCrop(object):
    ...
    def __call__(self, results):
        ...
        # if self.box_aug :
        #     ## 由于预测框可能不够准确，对 bbox 进行变动
        #     h, w = y2 - y1, x2 - x1
        #     if 'v_' in results['filename']:
        #         r = 0.05
        #         x1 += w * np.random.uniform(-r, r)
        #         y1 += h * np.random.uniform(-r, r)
        #         x2 = x1 + w * (np.random.uniform(-r, r) + 1)
        #         y2 = y1 + h * (np.random.uniform(-r, r) + 1)
        #     else:
        #         r = 0.05
        #         x1 += w * np.random.uniform(-r, r)
        #         y1 += h * np.random.uniform(-r, r)
        ...
        #w, h = x2 - x1 + 1, y2 - y1 + 1
        # crop 区域 x1 - offw,y1 - offh x2 + offw, y2 + offh
        offset_h = 32#np.random.randint(16, 64 + 1)
        offset_w = 32#np.random.randint(16, 64 + 1)
        crop_x1 = max(x1 - offset_w,0)
        crop_y1 = max(y1 - offset_h, 0)
        crop_x2 = min(x2 + offset_w, iw)
        crop_y2 = min(y2 + offset_h, ih)
        ...
        # cv2.rectangle(img, (x1, y1), (x2, y2), (0, 255, 0), 4)
        #
        # cv2.imwrite(p + "CenterCrop1" + os.path.basename(results['filename']), img)

        ih, iw = img.shape[:2]
        crop_h, crop_w = max(self.crop_size), min(self.crop_size)
        rr = min(crop_h / ih, crop_w / iw)
        if crop_h / ih > 2 and crop_w / iw > 2:
            img = mmcv.imrescale(img,2)
            x1, y1, x2, y2 = map(lambda x: x * 2, [x1, y1, x2, y2])
        elif rr > 1.0:
            img = mmcv.imrescale(img, rr)
            x1, y1, x2, y2 = map(lambda x: x * rr, [x1, y1, x2, y2])
        elif ih > crop_h or iw > crop_w:
            r = min(crop_h / ih, crop_w / iw)
            h_n, w_n = min(int(r * ih), crop_h), min(int(r * iw), crop_w)
            img = mmcv.imresize(img, (w_n, h_n))
            x1, y1, x2, y2 = map(lambda x: x * r, [x1, y1, x2, y2])
        ...
        #     x1, y1, x2, y2 = map(int, [x1, y1, x2, y2])
        #     cv2.rectangle(img, (x1, y1), (x2, y2), (0, 255, 0), 4)
        # cv2.imwrite(p + "AfterCenterCrop" +os.path.basename(results['filename']), img)
